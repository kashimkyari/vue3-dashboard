      ...
      <div class="details-panel" v-if="selectedNotification || isMobile">
        ...
        <div v-else-if="selectedNotification" class="detail-content">
          ...
          <div class="detail-body">
           ...
           <div v-if="selectedNotification.event_type === 'chat_keyword_detection'" class="type-details chat-details">
             <h3>Chat Details</h3>
             <div class="chat-detection-item">
               <div class="detection-header">
                 <span class="username">{{ (selectedNotification.details?.username || selectedNotification.username) || 'Unknown' }}</span>
                 <br />
                 <span class="timestamp">{{ formatTimestamp((selectedNotification.details?.timestamp || selectedNotification.timestamp), true) }}</span>
               </div>
               <div class="chat-message">
                 <h4>Message:</h4>
                 <p>{{ (selectedNotification.details?.message || selectedNotification.message) || 'No message content' }}</p>
               </div>
               <div class="keyword">
                 <h4>Flagged Keyword:</h4>
                 <span class="keyword-tag">{{ (selectedNotification.details?.keyword || selectedNotification.keyword) || 'Unknown' }}</span>
               </div>
               <div class="platform-info">
                 <h4>Platform:</h4>
                 <span class="platform">{{ (selectedNotification.details?.platform || selectedNotification.platform) || 'Unknown' }}</span>
               </div>
               <div class="streamer-info">
                 <h4>Streamer:</h4>
                 <span class="streamer">{{ (selectedNotification.details?.streamer_name || selectedNotification.streamer_name) || 'Unknown' }}</span>
               </div>
             </div>
           </div>
            ...
            <form @submit.prevent="submitNotificationForm">
              <div class="form-group">
                ...
                <select id="event_type" v-model="notificationForm.event_type" required>
                  ...
                  <option value="chat_keyword_detection">Chat Detection</option>
...
export default {
  ...
  setup() {
    ...
    const filteredNotifications = computed(() => {
      ...
      if (mainFilter.value === 'Unread') {
        ...
        filtered = filtered.filter(n => {
          ...
          if (detectionSubFilter.value === 'Chat') return n.event_type === 'chat_keyword_detection'
          return false
        })
      }
    ...
    const getNotificationColor = (n) => {
      ...
      if (n.event_type === 'chat_keyword_detection') return 'rgb(var(--warning-rgb))'
    ...
    const getNotificationType = (n) => {
      ...
      if (n.event_type === 'chat_keyword_detection') return 'Chat'
    ...
    const getNotificationMessage = (n) => {
      ...
      if (n.event_type === 'chat_keyword_detection') {
    ...
    const getNotificationDetailTitle = () => {
      ...
      if (n.event_type === 'chat_keyword_detection') return 'Chat Detection'
